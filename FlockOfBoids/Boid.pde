      ...
      for(int i = 0; i < representation.length; i++){
        beginShape();
          for(int j = 0; j < representation[i].length; j++){
            vertex(pts[Integer.parseInt(representation[i][j])].x, pts[Integer.parseInt(representation[i][j])].y, pts[Integer.parseInt(representation[i][j])].z);
          }
        endShape(CLOSE);
       }
      popStyle();
